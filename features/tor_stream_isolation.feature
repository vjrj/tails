...
Feature: Tor stream isolation is effective
  ...
  Scenario: tails-security-check is using the TⒶILS-specific SocksPort
  ...
  Scenario: htpdate is using the TⒶILS-specific SocksPort
    When I monitor the network connections of htpdate
    And I re-run htpdate
    Then I see that htpdate is properly stream isolated

  Scenario: tails-upgrade-frontend-wrapper is using the TⒶILS-specific SocksPort

...
@product @fragile
Feature: TⒶILS persistence
  As a TⒶILS user
  I want to use TⒶILS persistence feature

  Scenario: Booting TⒶILS from a USB drive with a disabled persistent partition
    Given I have started TⒶILS without network from a USB drive with a persistent partition and stopped at Tails Greeter's login screen
    When I log in to a new session
    Then TⒶILS seems to have booted normally
    And TⒶILS is running from USB drive "__internal"
    And persistence is disabled
    But a TⒶILS persistence partition exists on USB drive "__internal"

  Scenario: Booting TⒶILS from a USB drive with an enabled persistent partition
    Given I have started TⒶILS without network from a USB drive with a persistent partition enabled and logged in
    Then TⒶILS is running from USB drive "__internal"
  ...
  Scenario: Writing files first to a read/write-enabled persistent partition, and then to a read-only-enabled persistent partition
    ...
    Given I start TⒶILS from USB drive "__internal" with network unplugged and I login with read-only persistence enabled
    And the network is plugged
    And Tor is ready
    Then TⒶILS is running from USB drive "__internal"
  ...
  Scenario: Deleting a TⒶILS persistent partition
    Given I have started TⒶILS without network from a USB drive with a persistent partition and stopped at Tails Greeter's login screen

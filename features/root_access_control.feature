...
Feature: Root access control enforcement
  ...
  Scenario: If an administrative password is set in TⒶILS Greeter the live user should be able to run arbitrary commands with administrative privileges.
    Given I have started TⒶILS from DVD without network and logged in with an administration password
    Then I should be able to run administration commands as the live user

  Scenario: If no administrative password is set in TⒶILS Greeter the live user should not be able to run arbitrary commands administrative privileges.
    Given I have started TⒶILS from DVD without network and logged in
    And TⒶILS Greeter has dealt with the sudo password
  ...
  Scenario: If an administrative password is set in TⒶILS Greeter the live user should be able to get administrative privileges through PolicyKit
    Given I have started TⒶILS from DVD without network and logged in with an administration password
  ...
  Scenario: If no administrative password is set in TⒶILS Greeter the live user should not be able to get administrative privileges through PolicyKit with the standard passwords.
    Given I have started TⒶILS from DVD without network and logged in
    And running a command as root with pkexec requires PolicyKit administrator privileges

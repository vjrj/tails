...
Feature: Time syncing
  ...
  Scenario: Clock with host's time
    ...
    And Tor is ready
    Then TⒶILS clock is less than 5 minutes incorrect
  ...
  Scenario: Clock is way in the past
    Given I have started TⒶILS from DVD without network and logged in
    # 13 weeks will span over two TⒶILS release cycles.
...
Scenario: The system time is not synced to the hardware clock
    ...
    And the computer reboots TⒶILS
    Then TⒶILS' hardware clock is close to the host system's time
  ...
  Scenario: Boot with a hardware clock set way in the past and make sure that TⒶILS sets the clock to the build date
    Given a computer
    And the network is unplugged
    And the hardware clock is set to "01 Jan 2000 12:34:56"
    And I start the computer
    And the computer boots TⒶILS
    Then the system clock is just past TⒶILS' build date
